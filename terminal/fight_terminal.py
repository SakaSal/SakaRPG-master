...
def fight_terminal(stdscr, attacker, defender):
    # initiate colors
    # color for attacker
    blue_black=curses.init_pair(1, curses.COLOR_BLUE, curses.COLOR_BLACK)
    # color for defender
    green_black= curses.init_pair(3, curses.COLOR_GREEN, curses.COLOR_BLACK)
    # color for HP Bar
    cyan_red=curses.init_pair(2, curses.COLOR_CYAN, curses.COLOR_RED)
    # color for fight
    red_yellow=curses.init_pair(4, curses.COLOR_RED, curses.COLOR_YELLOW)
    # set colors
    attacker_color = blue_black
    defender_color = green_black
    hp_color = cyan_red
    fight_color = red_yellow
        ...
        if attacker_strikes > 0:
            ...
            if choice == "2":
                ...
                stdscr.addstr(
                    12,
                    0,
                    f"{attacker.name} parry's the next attack",
                )
        if defender_hp <= 0:
            in_fight = False
            break
        ...
        if defender_strikes > 0:
            ...
            if choice == "2":
                ...
                stdscr.addstr(
                    12,
                    0,
                    f"{defender.name} parry's the next attack",
                )
        if attacker_hp <= 0:
            in_fight = False
...
def draw_screen(attacker_hp, hp_color, defender_hp, fight_color, stdscr, fight_bar):
    # add HP boxes
    hp_bar_attacker = curses.newwin(1, 1 if attacker_hp <= 0 else attacker_hp, 3, 20)
    ...
    hp_bar_defender = curses.newwin(1, 1 if defender_hp <= 0 else defender_hp, 6, 20)
